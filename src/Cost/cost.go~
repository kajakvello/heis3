	...
	switch {
		...
		case orderFloor > myFloor:
			...
			cost += calculateCost(false, false, orderFloor, myFloor, 1, up, down, inside)
				...
				println("Something wrong in cost case 1")
			}
		...
		case orderFloor < myFloor:
			...
			cost += calculateCost(false, false, myFloor, orderFloor, 0, up, down, inside)
				...
				println("Something wrong in cost case 2")
			}
		...
		case orderFloor == myFloor:
			...
			if myDirection == orderDirection {
				...
				if myDirection == 0 {
					...
					println("Something wrong in cost case 3b")
				}
			}
...
//One order couts 4 points and one floor in difference counts 1 point
